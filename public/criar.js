...
function coletar(){
  return {
    ...
    foto: document.getElementById("p-foto").src && !document.getElementById("p-foto").hidden ? document.getElementById("p-foto").src : "",
...
function aplicar(d){
  ...
  document.getElementById("f-cpf").value = d.cpf || "";
  document.getElementById("f-nasc").value = d.nasc || "";
  ...
  const pf = document.getElementById("p-foto");
  if(d.foto){ pf.src = d.foto; pf.hidden = false; } else { pf.removeAttribute("src"); pf.hidden = true; }
...
// ---- foto (upload local, fica só no navegador) ----
function lerFoto(input){
  const f = input.files && input.files[0];
  if(!f) return;
  if(!f.type.startsWith("image/")){ alert("Escolha uma imagem."); return; }
  if(f.size > 1.5 * 1024 * 1024){ alert("Foto até 1,5MB pra não pesar o salvamento."); return; }
...
function atualizar(){
  ...
  const cpf = v("f-cpf"), nasc = document.getElementById("f-nasc").value;
  let nascFmt = "";
  if(nasc){ const p = nasc.split("-"); if(p.length === 3) nascFmt = p[2] + "/" + p[1] + "/" + p[0]; }
  ...
  const MODELOS = ["quadrado","faixa","lateral-direita","topo-amarelo","bege-limpo","compacto","duas-colunas","inverso","moldura-dupla","executivo","central","amarelo-total"];
...
// ---- PDF em modelo A4 (vazio sai como "Não informado") ----
function aplicarNaoInformado(){
  const marca = el => { if(el) el.textContent = "Não informado"; };
  const nome = document.getElementById("p-nome");
  if(nome && (!nome.textContent.trim() || nome.textContent === "Seu Nome Aqui")) marca(nome);
  const cargo = document.getElementById("p-cargo");
  if(cargo && (!cargo.textContent.trim() || cargo.textContent === "Seu Cargo Desejado")) marca(cargo);
  const contato = document.getElementById("p-contato");
  if(contato && (!contato.textContent.trim() || contato.textContent.includes("email • telefone"))) marca(contato);
  const resumo = document.getElementById("p-resumo");
  if(resumo && (!resumo.textContent.trim() || resumo.textContent.startsWith("Escreva seu"))) marca(resumo);
  const exp = document.getElementById("p-exp");
  if(exp && exp.textContent.includes("Clique em")) exp.innerHTML = "<p>Não informado</p>";
  else if(exp && !exp.textContent.trim()) exp.innerHTML = "<p>Não informado</p>";
  const edu = document.getElementById("p-edu");
  if(edu && edu.textContent.includes("Clique em")) edu.innerHTML = "<p>Não informado</p>";
  else if(edu && !edu.textContent.trim()) edu.innerHTML = "<p>Não informado</p>";
  const ski = document.getElementById("p-skills");
  if(ski && (ski.textContent.includes("Ex:") || !ski.textContent.trim())) ski.innerHTML = "<p>Não informado</p>";
  const c2 = document.getElementById("p-contato2");
  if(c2 && (c2.textContent.trim() === "—" || !c2.textContent.trim())) c2.innerHTML = "Não informado";
}
function imprimir(){
  atualizar();
  aplicarNaoInformado();
...
window.addEventListener("beforeprint", () => { try{ atualizar(); aplicarNaoInformado(); }catch(e){} });
window.addEventListener("afterprint", () => { try{ atualizar(); }catch(e){} });
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if(!temSalvo){ aplicar({ nome:"", cargo:"", cpf:"", nasc:"", email:"", tel:"", cidade:"", link:"", resumo:"", skills:"", cor:"marrom", modelo:"bege-limpo", exps:[{}], edus:[{}] }); }
  // modelo vindo da homepage: criar.html?modelo=faixa
  try{
    const q = new URLSearchParams(location.search).get("modelo");
    const VALIDOS = ["quadrado","faixa","lateral-direita","topo-amarelo","bege-limpo","compacto","duas-colunas","inverso","moldura-dupla","executivo","central","amarelo-total"];
  ...
  irParaPasso(1, false);
  const fi = document.getElementById("f-foto");
  if(fi) fi.addEventListener("change", () => lerFoto(fi));
  setTimeout(initAnimacoesCF, 0);
});

/* ===== PRETENSÃO (primeiro trabalho / mudar de emprego / estágio) ===== */
function escolherPretensao(modelo){
  const sel = document.getElementById("f-modelo");
  if(sel) sel.value = modelo;
  document.querySelectorAll(".pretensao-grade .btn").forEach(b => b.classList.remove("selecionado"));
  const btn = document.querySelector('.pretensao-grade .btn[onclick*="' + modelo + '"]');
  if(btn) btn.classList.add("selecionado");
  try{ if(typeof cfToast === "function") cfToast("■ Pretensão: " + modelo); }catch(e){}
  atualizar();
  irParaPasso(1, false);
}
